...
select distinct [dbo].fnRemoveSpecialCharacter(TaxShortName) as 'TaxShortName', IDENTITY(int,1,1) as ID into #TaxdataPM from InvoiceTaxDetail  where COALESCE(PATINDEX('%[^ ]%', [TaxShortName]), 0) > 0    
select distinct [dbo].fnRemoveSpecialCharacter(PaymentMode)as 'PaymentMode', IDENTITY(int,1,1) as ID into #PaymentDataPM from invoicepaymentdetails where COALESCE(PATINDEX('%[^ ]%', [PaymentMode]), 0) > 0    
